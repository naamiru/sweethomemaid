...
function skillEffects(
  ...
  if (skill === Skill.CrossRockets) {
  ...
  } else if (skill === Skill.V3Rockets) {
    for (let x = position[0] - 1; x <= position[0] + 1; x++) {
      for (const pos of vRocketRange(board, [x, position[1]], true)) add(pos)
    }
    ...
    add(position)
  } else if (skill === Skill.MixMissile) {
    boosterAs = Kind.Missile
    for (const pos of boosterRange(board, position, [
      Kind.Missile,
      Kind.Missile
    ])) {
      add(pos)
    ...
    add(position)
  } else if (skill === Skill.MixBomb) {
    boosterAs = Kind.Bomb
    for (const pos of boosterRange(board, position, [Kind.Bomb, Kind.Bomb])) {
      add(pos)
    }
    add(position)
  }
